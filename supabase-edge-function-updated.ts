...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Session ID único:', config.sessionId);
    console.log('Conteúdo do material disponível:', config.materialConteudo?.conteudoTexto ? 
      `Sim (${config.materialConteudo.conteudoTexto.length} caracteres)` : 'Não');
    ...
    // Usar valores únicos do cliente para garantir variabilidade
    const timestamp = config.timestamp || new Date().getTime();
    const randomSeed = config.randomSeed || Math.floor(Math.random() * 100000);
    const sessionId = config.sessionId || `${timestamp}-${randomSeed}`;
    
    // Prompts de variabilidade mais robustos
    const variabilityPrompts = [
      `Seja extremamente criativo e original (ID: ${sessionId})`,
      `Use abordagens completamente diferentes das anteriores (Seed: ${randomSeed})`,
      `Explore ângulos únicos e contextos inovadores (Time: ${timestamp})`,
      `Varie drasticamente o estilo e estrutura das questões (Session: ${sessionId})`,
      `Use exemplos e cenários totalmente distintos (Unique: ${randomSeed})`
    ];
    
    const selectedVariability = variabilityPrompts[randomSeed % variabilityPrompts.length];

    // Construir informações do material com conteúdo real
    let materialInfo = '';
    if (config.materialConteudo) {
      if (config.materialConteudo.conteudoTexto) {
        // Limitar conteúdo do material para não sobrecarregar o prompt (primeiros 3000 caracteres)
        const conteudoLimitado = config.materialConteudo.conteudoTexto.length > 3000 
          ? config.materialConteudo.conteudoTexto.substring(0, 3000) + "..." 
          : config.materialConteudo.conteudoTexto;
...
**IMPORTANTE:** Use este conteúdo como base principal para criar questões específicas e detalhadas sobre este material. As questões devem testar a compreensão deste conteúdo específico.`;
      } else {
        materialInfo = `
...
**IMPORTANTE - VARIABILIDADE EXTREMA:** ${selectedVariability}
**SESSÃO ÚNICA:** ${sessionId} - Esta deve ser uma geração COMPLETAMENTE NOVA e ORIGINAL.
**SEMENTE ALEATÓRIA:** ${randomSeed} - Use para garantir questões únicas.
**TIMESTAMP:** ${timestamp} - Contextualize questões no tempo atual.
...
**DIRETRIZES DE ORIGINALIDADE EXTREMA:**
- NUNCA repita padrões de questões anteriores
- Use vocabulário completamente diferente a cada geração
- Explore aspectos únicos do conteúdo do material fornecido
- Varie radicalmente a estrutura e abordagem
- Use contextos e cenários completamente novos
- Seja surpreendentemente criativo mantendo a relevância educacional
- Se há material base, foque em detalhes específicos desse conteúdo
...
- ${config.materialConteudo?.conteudoTexto ? 'MATERIAL ESPECÍFICO: Questões devem focar no conteúdo fornecido' : 'SEM MATERIAL: Questões gerais sobre os tópicos'}`;

    console.log('Prompt enviado para OpenAI (primeiros 1000 chars):', prompt.substring(0, 1000));

    // Chamar a API do OpenAI com configurações para mais variabilidade
    const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            ...
            IMPORTANTE: Cada sessão deve gerar questões COMPLETAMENTE DIFERENTES. Use criatividade máxima. 
        ...
        temperature: 0.9, // Alta variabilidade
        top_p: 0.95,
        frequency_penalty: 0.3, // Evitar repetições
        presence_penalty: 0.3,  // Encorajar novidade
        seed: randomSeed // Usar seed único do cliente
      })
    });
    ...
    return new Response(JSON.stringify({
      ...
      sessionId: sessionId,
      hasOriginalContent: !!config.materialConteudo?.conteudoTexto,
      config: {
        title: config.title,
